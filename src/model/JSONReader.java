...
public class JSONReader {
	...
	public static String manipulateJSON(WeatherInformation weather, String location) throws WeatherJsonError {
		...
		// Gets the LatLong with the MapQuest API
	...
	private static void jsonGridForecast(WeatherInformation weather, String url) {
		...
		// Load the WeatherInformation WeatherGridInformation instance variable with the related values
		weather.getWeatherGridInformation().forecastedTemp = gson.fromJson(temperature, WeatherGridInformation.ForecastedTemp.class);
		weather.getWeatherGridInformation().forecastedDewpoint = gson.fromJson(dewpoint, WeatherGridInformation.ForecastedDewpoint.class);
		weather.getWeatherGridInformation().dailyTemperatureHigh = gson.fromJson(maxTemperature, WeatherGridInformation.DailyTemperatureHigh.class);
		weather.getWeatherGridInformation().dailyTemperatureLow = gson.fromJson(minTemperature, WeatherGridInformation.DailyTemperatureLow.class);
		weather.getWeatherGridInformation().forecastedHumidity = gson.fromJson(relativeHumidity, WeatherGridInformation.ForecastedHumidity.class);
		weather.getWeatherGridInformation().forecastedApparentTemp = gson.fromJson(apparentTemperature, WeatherGridInformation.ForecastedApparentTemp.class);
		weather.getWeatherGridInformation().forecastedHeatIndex = gson.fromJson(heatIndex, WeatherGridInformation.ForecastedHeatIndex.class);
		weather.getWeatherGridInformation().forecastedWindChill = gson.fromJson(windChill, WeatherGridInformation.ForecastedWindChill.class);
		weather.getWeatherGridInformation().forecastedWindDirection = gson.fromJson(windDirection, WeatherGridInformation.ForecastedWindDirection.class);
		weather.getWeatherGridInformation().forecastedWindSpeed = gson.fromJson(windSpeed, WeatherGridInformation.ForecastedWindSpeed.class);
		weather.getWeatherGridInformation().forecastedWindGust = gson.fromJson(windGust, WeatherGridInformation.ForecastedWindGust.class);
		weather.getWeatherGridInformation().forecastedPreciptiationChance = gson.fromJson(probabilityOfPrecipitation, WeatherGridInformation.ForecastedPreciptiationChance.class);
	}
	 ...
	 * @throws WeatherJsonError 
	 */
	private static void jsonHourlyAndRegularForecast(ArrayList<WeatherPeriod> list, String url) throws WeatherJsonError {
		String json = readUrl(url);
		Gson gson = new Gson();
		JsonObject obj = JsonParser.parseString(json).getAsJsonObject();
		...
		if(properties != null) {
			...
			for (WeatherPeriod currPer : list) {
